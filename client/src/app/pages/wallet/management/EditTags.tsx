...
const EditTags = () => {
  ...
  const availableColors = Array.from(new Set(Object.values(categories).map((category) => category.color).filter(Boolean)));

  const handleDoubleClick = (id: string) => {
    setSelectedCategory(id);
    setEditedCategory({ name: categories[id].name, color: categories[id].color || '' });
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    if (selectedCategory) {
      try {
        ...
        await axios.patch(`http://localhost:5001/api/categories/${selectedCategory}`, editedCategory, config);
  ...
  const handleDelete = async () => {
    if (selectedCategory) {
      try {
        ...
        await axios.delete(`http://localhost:5001/api/categories/${selectedCategory}`, config);
  ...
  const handleAddCategory = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5001/api/categories', newCategory, config);
      if (response.status === 201) {
        alert('Category added successfully!');
        setCategories((prev) => ({
          ...prev,
          [response.data._id]: { name: response.data.name, color: response.data.color },
        }));
  ...
  return (
    <div className="py-6 px-16 bg-[hsl(var(--page-bg))] h-screen flex flex-col">
      ...
      <div className="text-base mt-2 mb-6">
        Double click to edit your tags
      </div>
        ...
        {isModalOpen && (
            ...
            >
                ...
                {availableColors.map((color) => (
                    ...
                    style={{ backgroundColor: color, border: editedCategory.color === color ? '2px solid black' : 'none' }}
                    onClick={() => setEditedCategory({ ...editedCategory, color: color || '' })}
                    />
                ))}
                </div>
              ...
              <div className="flex gap-2 mb-4">
                {availableColors.map((color) => (
                  <div
                    key={color}
                    className="w-8 h-8 rounded-full cursor-pointer"
                    style={{ backgroundColor: color, border: newCategory.color === color ? '2px solid black' : 'none' }}
